...
u8 MG200_Read_Cmd(u8 cmd, u8* result, u8* parameter)
{
	...
	if (MG200_Recv.data[0] != 0x6c)
		return 1;
	if (MG200_Recv.data[1] != 0x62)
		return 2;
	if (MG200_Recv.data[2] != 0x63)
		return 3;
	if (MG200_Recv.data[3] != cmd)
		return 4;
	...
	*parameter = MG200_Recv.data[5]; // 获取到返回的参数
	...
	if (MG200_Recv.data[7] != check_sum)
		return 6;
	return 0;
}
...
u8 CaptureAndExtract(u8 parameter)
{
	...
	{
	case 0x00:
		printf("抓取指纹图像及特征量提取成功！\r\n");
		break;
	case 0xb1:
		printf("指纹过小(small finger)！\r\n");
		break;
	case 0xb2:
		printf("无指纹(no finger)！\r\n");
		break;
	case 0xb3:
		printf("指纹位置过于靠左, 需要将手指靠右边摁按(left finger)！\r\n");
		break;
	case 0xb4:
		printf("指纹位置过于靠右, 需要将手指靠左边摁按(right finger)！\r\n");
		break;
	case 0xb5:
		printf("指纹位置过于靠上, 需要将手指靠下边摁按(up finger)！\r\n");
		break;
	case 0xb6:
		printf("指纹位置过于靠下, 需要将手指靠上边摁按(down finger)！\r\n");
		break;
	case 0xb7:
		printf("手指过湿(指纹表面出汗或者水渍过多)(wet finger)！\r\n");
		break;
	case 0xb8:
		printf("干燥手指(dry finger)！\r\n");
		break;
	case 0xc0:
		printf("特征提取错误！\r\n");
		break;
...
go2:
	...
	case 0x00:
		...
		break;
	case 0x83:
		printf("ID 错误(ID < 0 或 ID > 最大用户数)或通信错误\r\n");
		break;
	case 0x91:
		printf("注册失败(用户区域已满)\r\n");
		break;
