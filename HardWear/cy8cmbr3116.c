...
void CY8CMBR3116_Open_Door(void)
{
	...
	if(keynum != 0xff)
	{
		...
		{
			printf("结束:%s\r\n",CY_Pword.password);
			CY_Pword.password[CY_Pword.index] = '\0';
			if(strcmp((char*)CY_Pword.password,(char*)Key_PassWord) == 0)
			{
				Voice_SendCmd(0x12);//播报"欢迎回家"
				printf("开门成功\r\n");
				Door_Start(2000);
				memset(CY_Pword.password, 0, sizeof(CY_Pword.password));
				CY_Pword.index = 0;
			}
			else
			{
		...
		else if(keynum == '*')  // 删除键处理
        {
            if(CY_Pword.index > 0)  // 确保有字符可删除
            {
                CY_Pword.index--;  // 回退索引
                CY_Pword.password[CY_Pword.index] = '\0';  // 清除最后一个字符
                printf("删除一位: %s\r\n", CY_Pword.password);
            }
            else
            {
        ...
        {
            if(CY_Pword.index < sizeof(Key_PassWord) - 1)
            {
                CY_Pword.password[CY_Pword.index++] = keynum;
                printf("当前输入: %s\r\n", CY_Pword.password);
            }
            else
            {
